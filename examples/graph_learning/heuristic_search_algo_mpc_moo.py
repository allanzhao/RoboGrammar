...
def search_algo(args):
    ...
    # initialize the env
    env1 = RobotGrammarEnv(task1, rules, seed = args.seed, mpc_num_processes = args.mpc_num_processes)
    env2 = RobotGrammarEnv(task2, rules, seed = args.seed, mpc_num_processes = args.mpc_num_processes)
    ...
    if not args.test:
        ...
        fieldnames = ['rule_seq', 'reward1', 'reward2', 'reward', 'opt_seed1', 'opt_seed2']
        ...
        for epoch in range(args.num_iterations):
            ...
            if reward > best_reward:
                ...
                print_info('new best: reward = {:.4f}, predicted reward = {:.4f}, reward1 = {:.4f}, reward2 = {:.4f}, num_samples = {}'.format(reward, selected_reward, reward1, reward2, num_samples))
            ...
            for i in range(last_checkpoint + 1, len(designs)):
                writer.writerow({'rule_seq': str(designs[i]), 'reward1': design_rewards_1[i], 'reward2': design_rewards_2[i], 'reward': design_rewards[i], 'opt_seed1': design_opt_seeds_1[i], 'opt_seed2': design_opt_seeds_2[i]})
...
if __name__ == '__main__':
    ...
    args_list = ['--task1', 'FlatTerrainTask',
                 '--task2', 'FrozenLakeTask',
                 '--weight1', '0.5',
                 '--weight2', '0.5',
    ...
    if not args.test:
        args.save_dir = os.path.join(args.save_dir, "{}_{}".format(args.task1, args.task2), "{:.1f}_{:.1f}".format(args.weight1, args.weight2), get_time_stamp())
        try:
