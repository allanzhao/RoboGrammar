...
def find_applicable_matches(rule, target):
  return list(filter(
      lambda match: rd.check_rule_applicability(rule, target, match),
      rd.find_matches(rule.lhs, target)))

rule_id_sequence = list()
while True:
  rule_matches = [(rule, find_applicable_matches(rule, robot_graph))
                  for rule in rules]
  # Filter out rules that cannot be applied
  rule_matches = list(filter(lambda item: item[1], rule_matches))
  if rule_matches:
    rule, matches = random.choice(rule_matches)
    rule_id_sequence.append(rules.index(rule))
    # Always use the first match
    robot_graph = rd.apply_rule(rule, robot_graph, matches[0])
  else:
    break

print("Rule ID sequence:", rule_id_sequence)

robot_nodes = robot_graph.nodes
for node in robot_nodes:
  if node.attrs.shape == rd.LinkShape.NONE:
    node.attrs.shape = rd.LinkShape.CAPSULE
robot_graph.nodes = robot_nodes
